...
## Отложено — нагрузка демонстрации без снижения качества

- Наблюдение владельца: во время демонстрации у Discord около 2–3%, у Voople
  5% и выше. Это ещё не сопоставимый замер: не зафиксированы метрика, настройки,
  источник, preview и состав процессов. Причина пока неизвестна.
- Приоритет ниже ядра, стабильности и текущей переработки. Сами эти цифры не
  блокируют релиз. Зависания, перегрев или подтверждённая существенная регрессия
  в рабочем сценарии требуют отдельного повышения приоритета.
- Сначала воспроизвести сравнение на одном устройстве: одинаковый источник,
  разрешение/FPS, число зрителей, длительность, состояние preview и окна.
  Записать версии ОС/драйвера/клиентов, доступные codec/bitrate и ограничения
  сравнения. Измерять всё дерево процессов приложения, включая native worker,
  а не только один renderer.
- Собрать CPU, GPU encode/copy/render, память, фактические FPS/dropped frames,
  bitrate и задержку. Разделить capture, копирование кадров, encoding,
  отправку и локальный preview. Проверить аппаратное ускорение и лишнюю работу
  при скрытом preview/свёрнутом окне до выбора оптимизации.
- Результат: воспроизводимый baseline и замеры до/после, снижение подтверждённой
  нагрузки без ухудшения согласованных качества, FPS, задержки и звука;
  regression-check stop/restart/reconnect. Не подменять оптимизацию снижением
  пресета и не обещать одинаковый процент с Discord на любом железе.
- Для профиля использовать тестовую сцену. Не публиковать приватные кадры,
  заголовки чужих окон, токены или media URLs в trace/отчёте.
