...
| 2 — Messaging | `Nav → Chat List → Conversation`, `+` внутри strip разделов, Members/Info drawers по запросу, presence/room context, единые header/composer/menus/Room CTA и безопасное закрытие conversation через active `Чаты`/Escape | Основной flow и shared tooltip/icon controls composer/group header есть; active `Чаты` и `Escape` возвращают в inbox, overlay/selection поглощают первый `Escape`, чтение отделено от query и ограничено последним реально показанным сообщением в видимом/focused окне; attachment menu всегда даёт фото и условно — playlist music | Тот же exit/read contract; unmount останавливает realtime channel, polling и stale load, draft остаётся локально; внешний thread/messages controller ещё расходится | Нужны right Members drawer с `Сейчас/В сети/Все/Роли`, section-strip `+`, parity snapshots, keyboard/context menu/upload states и authenticated tooltip visual gate | Частично |
| 3 — Room | Full, Share, Empty, Mini, Compact, Minimal, mobile как одна state machine | Состояния существуют; web-share использует общий stage, единые icon controls/tooltips и явные 720p30/1080p60 presets | Native LiveKit/libwebrtc изолирован в worker process; UI stop мгновенный, graceful unpublish предшествует blocking join/forced kill, late session/track events изолированы; self-preview opt-in, unfocused preview приостанавливается; общий stage задаёт полный video box, `object-fit: contain` сохраняет весь кадр, оставшаяся высота и content-bound fullscreen остаются под desktop chrome; Room header/footer/media/dock используют shared controls | Source contract проверяет полный `width/height: 100%` video box вместе с `contain`; regression с маленьким кадром на большом чёрном canvas закрыт в focus/grid/mini коде. Остаются production-подтверждение этого RC, воспроизводимый двухклиентный stop/restart/quality gate, fullscreen/16:10/ultrawide/portrait visual matrix, reconnect/soak, фактические FPS/bitrate, screen-reader/keyboard/disabled-tooltip visual gate и все visual states | Частично |
...
| 45 | Shared icon controls and tooltips | Частично | Общие `Tooltip`/`IconButton`, viewport flip/clamp с учётом native titlebar, hover-delay, focus/Escape, touch suppression, reduced-motion и aria-label подключены к compact sidebar, Room header/footer/media/dock, «Новому разделу» и «Комнате», composer/group-header controls; source/unit contracts зелёные. Остались trigger-owned account tooltip без утечки над portalled menu, инвентаризация остальных icon-only действий, authenticated web/desktop visual check, screen-reader pass и единый паттерн объяснения disabled-состояний |
| 46 | Board-by-board convergence | Частично | Safe conversation close и bounded read lifecycle Board 2 реализованы общими hooks/contracts для web/desktop и покрыты source/unit checks. Далее Board 2 закрывается section-strip `+`, Members overlay drawer с presence/Room/roles и полной responsive/accessibility/visual матрицей; затем по тому же gate Boards 1, 3, 4, 5 и 6 |
...
| 48 | Runtime/auth resilience | Частично | Client tRPC больше не зависит от прямого Node `process`; exact `JWT issued at future` и auth transport получают bounded retry, legal consent автоматически повторяет только `SERVICE_UNAVAILABLE`. Нужны cold/stale/expired/offline→online/VPN visual/E2E gates и session-bootstrap timeout state |
| 49 | Room surface continuity | Планирование · P1 UX | Объединить prejoin/connecting/active/reconnecting/leaving/post-leave в одну геометрически стабильную shared surface без modal→sheet скачка; inline retry, stale-state barriers, focus restore, reduced-motion и web/desktop/small-window/fullscreen visual matrix |
