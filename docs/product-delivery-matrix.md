...
| 53 | Room-context messages | Частично · P1 core | Сообщения остаются в Group Chat: migration 66 атомарно привязывает immutable Room/LiveSession snapshot в транзакции создания сообщения, выводя Room только из активной same-Group session отправителя. Клиент не передаёт Room ID. Общий read model совместим с ещё не обновлённой БД, разделяет визуальные группы при смене Room и показывает спокойную метку контекста в существующем message bubble без второй ленты. Migration 66 применена и зарегистрирована в настроенной Supabase 2026-09-09. Реализованная боковая панель с точным LiveSession-фильтром теперь считается временной совместимостью: Full Room должен переиспользовать обычную историю выбранной Group/Section без Room/session filter и без второго draft/read lifecycle. Остаются общая Conversation composition для main/drawer, поиск/навигация к Room-контексту, реальный DB concurrency gate и authenticated web/desktop/mobile evidence |
