...
Закрытие панели возвращает focus к её trigger. Source/unit, TypeScript, targeted
lint и architecture gates зелёные; authenticated визуальная проверка этих
порогов не заявляется. Следующий P3-срез объединит reply, upload, edit/reaction
и desktop transport state с основной Conversation composition.
