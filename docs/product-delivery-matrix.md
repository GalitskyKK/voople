...
Закрытие панели возвращает focus к её trigger. Main conversation и drawer на
web используют один conversation-keyed composer store: text, reply, edit,
готовый upload и выбранный track не сбрасываются при minimize или переходе
...
этих порогов не заявляется. Следующий P3-срез подключит тот же contract к
desktop main transport/composer state.
