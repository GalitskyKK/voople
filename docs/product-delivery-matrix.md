...
| 66 | Voice-first Switch / Split / Voop delta | Спецификация · P0 core | Decision Memo, IA/UI Spec и Implementation Brief синхронизированы с `last_try`: Room card целиком выполняет Switch без отдельного Join и без auto-Full; Split создаёт temporary разговор без setup и сначала перемещает только инициатора; `Вуп` является действием на человеке и создаёт Split только после явного accept; `+ Комната` отделена как deliberate permanent/pinned creation; Leave принадлежит current Room, Full открывается по Expand, Mini — после явного сворачивания. Реализация, server-authorized Voop invite lifecycle, responsive states и authenticated web/desktop evidence ещё не готовы. |
