...
| 54 | Full Room invitations | Частично · P0 Room | Migration 58 переиспользована как service-role-only session-bound store. Только актуальный участник точной core Room получает privacy-filtered список участников группы и отправляет идемпотентный 15-minute invite под отдельным rate limit. Shared Room sheet web/desktop показывает поиск, loading/empty/error/retry и server-owned pending/accepted/declined/expired/cancelled status; pending invite можно атомарно и идемпотентно отменить только его отправителю из той же активной session. Notification batch-read повторно проверяет invitee, session, group/Room consistency и `roomsScope`, отображает все статусы и принимает/отклоняет invite. Вход использует общий cross-context confirmation + token/media handoff, а `accepted` сервер разрешает только после фактического join в точную session. Остались block-модель, внешний authenticated deep-link/share preview, production migration evidence и multi-user responsive visual/E2E |
