...
| 61 | Core rework foundation + Group Now contracts | Частично · P0 architecture | Tracked source gate, ADR, typed feature availability registry и additive schema дополнены server-owned read model для Lobby/Rooms/LiveSession participants/Online и service-role-only mutation boundary. Root membership/privacy/current-member filtering и dual-read legacy compatibility применяются до View; SQL-транзакция сериализует actor/target Room, различает same-group и cross-context switch, использует session-bound stale-safe leave/heartbeat и grace lifecycle временной Room. Legacy enter отклоняет параллельную новую LiveSession; публичного UI/API switch нет. Source/TypeScript gates зелёные. До следующего статуса нужны реальный Postgres concurrency test, capability-gated transport/full UI states, release migration evidence и old-client/two-client production gate |
