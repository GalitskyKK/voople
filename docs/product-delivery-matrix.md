...
| 3 — Room | Full, Share, Empty, Mini, Compact, Minimal, mobile как одна state machine | Состояния существуют; shared sheet сохраняет одну геометрию для loading/preview/connecting/inside/reconnecting/leaving/error, web-share использует общий stage, единые icon-only media controls/tooltips до и после входа и явные 720p30/1080p60 presets | Native LiveKit/libwebrtc изолирован в worker process; явные transition barriers не показывают stale stage при connect/leave, leave ждёт server refetch; UI stop мгновенный, graceful unpublish предшествует blocking join/forced kill, late session/track events изолированы; self-preview opt-in, unfocused preview приостанавливается; общий stage задаёт полный video box, `object-fit: contain` сохраняет весь кадр, оставшаяся высота и content-bound fullscreen остаются под desktop chrome; Room header/footer/media/dock используют shared controls | Source contracts проверяют phase precedence, стабильную sheet geometry, connect/leave barriers, полный `width/height: 100%` video box вместе с `contain` и icon-only media controls. Остаются production-подтверждение RC, воспроизводимый двухклиентный stop/restart/quality gate, fullscreen/16:10/ultrawide/portrait visual matrix, reconnect/soak, фактические FPS/bitrate, focus restore, screen-reader/keyboard/disabled-tooltip gate | Частично |
...
| 49 | Room surface continuity | Частично · P1 UX | Shared state resolver и одна sheet geometry покрывают loading/preview/connecting/inside/reconnecting/leaving/error; explicit connect/leave transition не даёт stale server state вернуть старый экран, leave удерживается до refetch, активная чужая Room показывает участников до входа, а connected stage не скрывается из-за degraded heartbeat. Остались inline retry action в content, post-leave confirmation policy, focus restore, reduced-motion и authenticated web/desktop/small-window/fullscreen visual matrix |
