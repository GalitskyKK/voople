...
| 65 | Messenger/live IA replacement | В работе · P1 messenger | Decision Memo, IA/UI Spec и Implementation Brief приняты как верхний decision set; source hierarchy, architecture и integration plan синхронизированы. Group открывается в Chat; Join должен открыть Full Room в main content без modal backdrop, global sidebar остаётся; переход в другой контекст сохраняет ту же session как Mini; contextual drawer показывает обычный выбранный Group/Section Chat без LiveSession-фильтра. Bounded Shelf держит Lobby первым, показывает 3/2/1 Room по ширине, до трёх avatar tokens, count, live/share/current state, прямое действие и подписанный portalled overflow; пустой Shelf не рендерится. Root Group на wide desktop объединяет identity, `Чат / Сейчас / Люди` и Lobby action в один 64 px header; group color больше не подменяет system violet. Sections больше не образуют horizontal scroller: текущий раздел открывает один searchable dialog-picker, layout `tabs` даёт максимум два shortcuts, а `+` остаётся в той же 36/44 px toolbar. Общий web/desktop renderer выводит Group и DM единым ограниченным по ширине левым dense stream: avatar/автор/Room snapshot один раз на 5-minute block, свои помечены `вы` и delivery ticks, replies разрывают блок, а текст больше не получает отдельную bubble-card; вложения, reactions, reply, edit/delete и selection сохранены. Общая conversation-state surface покрывает initial loading, offline и error; при stale history история и composer не исчезают, retry находится в компактной полосе, а desktop очищает ошибку после успешного background recovery. Targeted native tests, architecture, targeted lint и web TypeScript зелёные. Desktop build не стартовал из-за отсутствующего локального `vite/client`; responsive visual evidence не заявляется. Впереди remaining Section unread/favorites/keyboard semantics и responsive visual gate, затем P2 Full/Mini, P3 conversation drawer/screen share и P4 guest acceptance. |
