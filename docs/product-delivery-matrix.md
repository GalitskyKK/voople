...
| 10 | Единый Room CTA | Shared presentation используется на части surfaces; root-group header и Group Info в web/desktop теперь используют один `GroupRoomAction`, который fail-closed переключается между stable legacy Room и internal multi-Room panel | Один contract/state в home/notification/invite/public-group surfaces, live-count label и authenticated responsive/keyboard visual gate | Частично |
