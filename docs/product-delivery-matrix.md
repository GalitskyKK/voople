...
| 65 | Messenger/live IA replacement | В работе · P0 source gate | Decision Memo, IA/UI Spec и Implementation Brief приняты как верхний decision set. Group открывается в Chat; Lobby и занятые Room появляются bounded Live Shelf; Join открывает Full Room в main content без modal backdrop, global sidebar остаётся; переход в другой контекст сохраняет ту же session как Mini; contextual drawer показывает обычный выбранный Group/Section Chat без LiveSession-фильтра. Source hierarchy, architecture и integration plan синхронизированы. Это не закрывает продуктовый slice: впереди P1 messenger, P2 Full/Mini continuity, P3 conversation drawer/screen share и P4 guest acceptance со всеми state, responsive, parity и real-client gates. |
