...
этот cache и больше не поддерживает вторую локальную копию переписки. Отдельным
несведённым слоем остаётся desktop track UI.
