...
| 65 | Messenger/live IA replacement | В работе · P1 messenger | Decision Memo, IA/UI Spec и Implementation Brief приняты как верхний decision set; source hierarchy, architecture и integration plan синхронизированы. Group открывается в Chat; Join должен открыть Full Room в main content без modal backdrop, global sidebar остаётся; переход в другой контекст сохраняет ту же session как Mini; contextual drawer показывает обычный выбранный Group/Section Chat без LiveSession-фильтра. Bounded Shelf держит Lobby первым, показывает 3/2/1 Room по ширине, до трёх реальных avatar tokens, count, live/share/current state, прямое действие и подписанный portalled overflow; пустой Shelf не рендерится. Root Group на wide desktop объединяет identity, `Чат / Сейчас / Люди` и Lobby action в один 64 px header, а narrow/selection/section сохраняют безопасный двухрядный layout. Group accent больше не подменяет системный theme accent: controls остаются violet, group color принадлежит identity/content. 9 native source/unit tests, architecture, targeted lint и web TypeScript зелёные. Desktop build не стартовал из-за отсутствующего локального `vite/client`; responsive visual evidence не заявляется. Впереди section selector/dense stream и state surfaces, затем P2 Full/Mini, P3 conversation drawer/screen share и P4 guest acceptance. |
