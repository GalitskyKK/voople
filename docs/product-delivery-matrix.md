...
| 2 — Messaging | `Nav → Chat List → Conversation`, `+` внутри strip разделов, Members/Info drawers по запросу, presence/room context, единые header/composer/menus/Room CTA и безопасное закрытие conversation через active `Чаты`/Escape | Основной flow и shared tooltip/icon controls composer/group header есть; действия «Новый раздел» и «Комната» компактны и используют общий accessible tooltip; attachment menu всегда даёт фото и условно — playlist music | Те же shared presentation и controls; внешний thread/messages controller ещё расходится | Нужны right Members drawer с `Сейчас/В сети/Все/Роли`, section-strip `+`, read-observer teardown при закрытии, parity snapshots, keyboard/context menu/upload states и authenticated tooltip visual gate | Частично |
...
| 46 | Board-by-board convergence | Планирование | Для Boards 1–6 нужен отдельный gap map и закрываемый по одному полный slice: data/auth, shared View, web/desktop adapters, states, responsive/accessibility и визуальная матрица. Board 2 включает section-strip `+`, Members overlay drawer с presence/Room/roles и safe conversation close |
