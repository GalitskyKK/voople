...
| 4 | «Сейчас» | Online/listening, accepted DM и люди из общих групп дополнены отдельной карточкой каждой активной core Room без схлопывания группы. Core query закрыт тем же server capability/user allowlist, поэтому stable-клиент не зависит от новых таблиц до rollout. Read model отсекает stale/left участников, повторно проверяет актуальное membership и `roomsScope`, дедуплицирует legacy/core participant presence и исключает conversation с active Room из «Продолжить». Shared web/desktop CTA действительно подключает Room через общий session-bound join/media handoff; cross-context confirmation сохраняется, а `presence_room_joined` пишется только после успешного открытия legacy/core media surface | Game/activity, недавнее взаимное общение без DM, loading/retry для live refresh, authenticated responsive/empty visual gate и реальные multi-user live tests | Частично |
...
| 10 | Единый Room CTA | Root-group header и Group Info используют один `GroupRoomAction`, который fail-closed переключается между stable legacy Room и internal multi-Room panel. Home использует тот же dynamic join coordinator/token/compensating-leave lifecycle для конкретной Room и один общий controller на web/desktop; прямой legacy-разговор остаётся типизированным отдельным context, а не маскируется под group | Подключить тот же contract/state к notification/invite/public-group surfaces, добавить live refresh/count labels и authenticated responsive/keyboard visual gate | Частично |
...
| 61 | Core rework foundation + Group Now contracts | Частично · P0 architecture | Tracked source gate, ADR, typed feature registry и additive schema дополнены server-owned Group Now read model, service-role-only atomic mutations и fail-closed internal tRPC transport. Root membership/privacy/current-member filters применяются до View; SQL сериализует actor/target Room, различает same-group/cross-context switch и использует session-bound stale-safe leave/heartbeat + grace lifecycle. Legacy enter учитывает новую LiveSession. Transport требует internal channel, `multi_room_groups` capability и user allowlist, валидирует input, rate-limit-ит записи и не пишет private IDs в telemetry. Shared web/desktop Group Now View использует плоские Room sections, canonical portable avatars, max-width live-content и loading/quiet/error/offline/action-error states; controller по умолчанию `enabled=false`. Shared join coordinator теперь принимает dynamic group/Room target и поэтому обслуживает Group Now и global Home без второго join/token/cleanup implementation: сначала отправляет `confirmedCrossContext=false`, показывает confirmation только на server `PRECONDITION_FAILED`, повторяет с explicit consent и muted mic, затем выполняет session-bound token exchange; disabled credential или provider handoff error компенсируются exact-session leave. Create dialog различает временные и admin-only закреплённые Room, создаёт один request UUID на submit и повторно использует его только после явного cross-context consent; `create + join` проходит одним RPC. Read model несёт session start/starter и `isMe`, а чистый core-to-voice mapper валидирует точный session ID и приводит участников к существующему `ChatRoomView`. Общий heartbeat hook принимает типизированную legacy/core цель; core-вариант привязан к session ID, передаёт mic/camera/screen state через существующий таймер и не пишет private IDs в error log. VoiceSession server adapter подключает Group Now к существующему `VoiceSessionProvider` и `ChatRoomControl`: initial credential живёт только в одноразовом ref, reconnect получает новый session-bound token, leave/heartbeat используют точную LiveSession. Core media credentials и нативный desktop screen-audio token выводят provider-room только из server-owned session, требуют active participant + root membership и ограничивают grants/TTL. Root-group header, Group Info и Home подключены к shared Room lifecycle; Home batch-read сохраняет несколько Room одной группы, проверяет recent participant heartbeat/current membership/privacy и не выдаёт пустые sessions. Migration 61 добавляет idempotent request UUID и один service-role-only `create + join` statement: confirmation error откатывает Room insert, а retry потерянного ответа не создаёт дубль. Выделенный Postgres integration gate проверяет concurrency/idempotency/rollback только через безопасный `VOOPLE_TEST_DATABASE_URL`; в текущем окружении он skipped из-за production-like Supabase URL, поэтому реальное DB evidence ещё не засчитано. Остались notification/public-group/invite Room CTA wiring, live Home refresh, responsive visual evidence, отдельный Postgres gate, release migration evidence и old-client/two-client production gate |
