...
| 61 | Core rework foundation + Group Now read model | Частично · P0 architecture | Tracked source gate, ADR, typed feature availability registry и additive schema дополнены server-owned read model для Lobby/Rooms/LiveSession participants/Online. Root membership, `roomsScope`, `onlineScope`, current-member filtering и dual-read legacy compatibility проверяются до сборки View; новая LiveSession побеждает дублирующий legacy heartbeat. Публичного UI/API switch нет. До следующего статуса нужны capability-gated transport и full UI states, atomic create/join/switch/leave с concurrency tests, release migration evidence и old-client production gate |
