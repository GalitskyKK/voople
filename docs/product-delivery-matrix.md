...
двух Room при переходе по ссылкам. Ответ 401/410 на гостевой heartbeat теперь
отключает медиа и возвращает актуальный preview; временную сетевую ошибку он
не трактует как окончание разговора. Это не заявляет полный end-to-end B без
...
Checkpoint Group/Room invite (2026-09-23): общий web/desktop Group header
показывает `Пригласить` и по обычному нажатию копирует membership-ссылку
на 7 дней без модального окна; Room `Позвать` остаётся отдельной гостевой
ссылкой. Восстановление гостевой сессии/выдача нового media token и heartbeat
проверяют исходное приглашение (точную LiveSession, отзыв, истечение),
терминальный ответ отключает клиентский LiveKit. Source/unit regressions пройдены;
реальный guest media, revoke/expiry race, web/desktop/360 px и SQL RPC
остаются неподтверждёнными. Миграции 73–76 в настроенной базе на этот
checkpoint не применены.
