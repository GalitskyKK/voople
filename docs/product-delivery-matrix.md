...
| 65 | Messenger/live IA replacement | В работе · P1 messenger | Decision Memo, IA/UI Spec и Implementation Brief приняты как верхний decision set; source hierarchy, architecture и integration plan синхронизированы. Group открывается в Chat; Join должен открыть Full Room в main content без modal backdrop, global sidebar остаётся; переход в другой контекст сохраняет ту же session как Mini; contextual drawer показывает обычный выбранный Group/Section Chat без LiveSession-фильтра. Первый P1-срез заменяет horizontal chip scroller bounded Shelf: Lobby стабильно первый, при доступной ширине видны 3/2/1 Room, каждая показывает до трёх реальных avatar tokens, count, live/share/current state и прямое действие, остальные доступны через подписанный portalled overflow menu. Пустой Shelf не рендерится. 9 native source/unit tests, architecture, targeted lint и web TypeScript зелёные. Desktop build не стартовал из-за отсутствующего локального `vite/client`; responsive visual evidence не заявляется. Впереди wide header/section selector/dense stream и state surfaces, затем P2 Full/Mini, P3 conversation drawer/screen share и P4 guest acceptance. |
