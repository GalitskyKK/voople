...
- **Баннер:** редактирование профиля → загрузка / рисование (`setCustomBanner`).
- **Эффект, кольцо, стиль ленты и др.:** `/shop?tab=customize` или ссылка «Оформление профиля» в `ProfileEditSheet`. Предметы из каталога + экипировка (`CustomizationEditor`).
- Свои загружаемые эффекты (не из магазина) — в планах, см. [customization.md](./customization.md#user-created-assets).

### Подписка

`ProfileFollowButton`: бейдж **Подписка** при `following`, подпись **Подписан на вас** / **Взаимно**, кнопка **Отписаться** (secondary) или **Подписаться** / **В ответ** (primary).
