...
Порядок учитывает `rework_plan/VOOPLE_CORE_REWORK_PLAN.md` и обязательные
поправки из `rework_plan/VOOPLE_CORE_REWORK_ADDENDUM.md`: повседневный
мессенджер — главный сценарий, комнаты — его live-слой. При конфликте
дополнение имеет приоритет. Старые discovery/экономические задачи не должны
вытеснять ядро.
