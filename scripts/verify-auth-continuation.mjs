...
const mocks = {
  ...
  "@/lib/auth/sync-public-user": `export const syncPublicUser=async()=>{if(window.scenario.syncFail)throw new Error('private sync details');return {username:'test_user',created:window.scenario.created}};`,
  ...
  "@/lib/supabase/client": `export const createClient=()=>({auth:{
    ...
    signUp:async credentials=>{window.emailRedirectTo=credentials.options?.emailRedirectTo;return window.scenario.fail?{error:{message:'Тест: сеть недоступна'}}:{data:{session:window.scenario.confirm?null:{access_token:'fixture-only'}}}},
    exchangeCodeForSession:async()=>{window.exchanges++;if(window.scenario.confirmationPending)await new Promise(resolve=>window.finishConfirmation=resolve);if(window.scenario.confirmationFail)return {data:{session:null},error:{message:'private confirmation details'}};window.scenario.confirmedSession=true;return {data:{session:{access_token:'fixture-only'}},error:null}},
    getSession:async()=>({data:{session:window.scenario.confirmedSession?{access_token:'fixture-only'}:null},error:null}),
    getUser:async()=>({data:{user:window.scenario.confirmedSession?{}:null},error:window.scenario.confirmedSession?null:{message:'private session details'}})
  }});`,
...
const entry = `import {StrictMode,useSyncExternalStore} from 'react';import {createRoot} from 'react-dom/client';
  import Login from '@/app/(auth)/login/page';import Register from '@/app/(auth)/register/page';import EmailConfirmation from '@/app/(auth)/auth/confirm/page';
  ...
  window.scenario={};window.exchanges=0;window.navigate=href=>{history.pushState({},'',href);dispatchEvent(new PopStateEvent('popstate'))};
  const subscribe=fn=>{addEventListener('popstate',fn);return()=>removeEventListener('popstate',fn)};
  function App(){const current=useSyncExternalStore(subscribe,()=>location.pathname+location.search,()=>'');const theme=useAppTheme();window.changeTheme=theme.setThemeId;
    const params=new URLSearchParams(location.search);return <div key={current} style={{minHeight:'100vh',background:'var(--background)',color:'var(--foreground)',display:'grid',placeItems:'center',padding:16}}>
      {location.pathname==='/login'?<Login/>:location.pathname==='/register'?<Register/>:location.pathname==='/auth/confirm'?<EmailConfirmation/>:location.pathname==='/onboarding'?<OnboardingFlow username={params.get('username')||'test_user'} redirectAfter={safeAuthContinuation(params.get('redirect'))||undefined}/>:<p>Destination reached</p>}</div>}
...
try {
  ...
  for(const width of [360,1280]) for(const theme of ["void","light"]){
    ...
    const callback=new URL(await page.evaluate(()=>window.emailRedirectTo));assert.equal(callback.pathname,"/auth/confirm");assert.equal(callback.searchParams.get("redirect"),invite);
    const callbackPath=`${callback.pathname}${callback.search}&code=fixture-code`;
    await page.evaluate(path=>{window.scenario={confirmationPending:true,created:true};window.navigate(path)},callbackPath);
    await page.getByText("Проверяем ссылку…",{exact:true}).waitFor();await screenshot("confirmation-pending");assert.equal(await page.evaluate(()=>window.exchanges),1);
    await page.evaluate(()=>window.finishConfirmation());await page.waitForURL(/\/onboarding\?/);assert.equal(new URL(page.url()).searchParams.get("redirect"),invite);
    await page.evaluate(path=>{window.scenario={confirmationFail:true,created:true};window.navigate(path.replace('fixture-code','expired-code'))},callbackPath);
    await page.getByText("Не удалось подтвердить почту",{exact:true}).waitFor();await screenshot("confirmation-error");
    assert.equal(await page.getByText("private confirmation details",{exact:true}).count(),0);assert.equal(new URL(await page.getByRole("link",{name:"Перейти ко входу"}).getAttribute("href"),origin).searchParams.get("redirect"),invite);
    await page.evaluate(()=>{window.scenario={created:true}});await page.getByRole("button",{name:"Повторить",exact:true}).click();await page.waitForURL(/\/onboarding\?/);assert.equal(await page.evaluate(()=>window.exchanges),3);
    await page.evaluate(login=>{window.scenario={};window.navigate(login)},login);
    ...
    assert.deepEqual(errors,[]);await page.close();console.log(`PASS ${width} ${theme}: links/reload, password pending/error/retry, email callback pending/error/retry, OTP, onboarding, unsafe redirect`);
  }
