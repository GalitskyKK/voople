...
const entry = `import {useState} from 'react';import {createRoot} from 'react-dom/client';
  ...
  const rooms=[
    {id:'lobby',kind:'lobby',name:'Лобби',joinTarget:{kind:'room',roomId:'lobby'},state:'active',liveSessionId:'s1',startedAt:new Date(Date.now()-36*60e3).toISOString(),startedBy:'biba',participantCount:3,hasScreenShare:false,participants:users.slice(0,3)},
    {id:'drg',kind:'pinned',name:'DRG',joinTarget:{kind:'room',roomId:'drg'},state:'active',liveSessionId:'s2',startedAt:new Date(Date.now()-24*60e3).toISOString(),startedBy:'nmggk',participantCount:1,hasScreenShare:true,participants:[users[3]]}
  ];
...
const bundle = await build({
  stdin: { contents: stagedEntry, resolveDir: repo, loader: "tsx" }, bundle: true, write: false,
