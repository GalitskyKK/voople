  ...
  return (
    <>
      ...
      <div className="voople-group-header-actions ml-auto flex shrink-0 items-center gap-2">
        ...
        {canManage ? <button type="button" onClick={onInvite} className="voople-group-header-invite hidden h-12 shrink-0 items-center gap-2.5 px-5 text-sm font-semibold sm:inline-flex"><UserPlus className="h-3.5 w-3.5" aria-hidden="true" />В группу</button> : null}
      ...
      <Sheet open={open} onClose={() => onOpenChange(false)} placement="right" ariaLabel={`Информация о группе ${chatName}`}>
        ...
        {!error && tab === "info" ? (
          <div className="mt-5 space-y-4 text-sm leading-6 text-[var(--app-muted)]">
            ...
            {canManage ? <div className="border-t border-[var(--app-border)] pt-4"><button type="button" onClick={onInvite} className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[var(--theme-accent)] px-3 text-sm font-semibold text-slate-950"><UserPlus className="h-4 w-4" />В группу</button></div> : null}
          </div>
        ) : !error ? (
