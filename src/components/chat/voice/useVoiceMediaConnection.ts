...
import { ConnectionCheck, ConnectionQuality, ConnectionState, Room } from "livekit-client"
...
export function useVoiceMediaConnection({
  ...
  const connect = async () => {
    console.log("[VOICE-7] useVoiceMediaConnection.connect()", {
      hasExistingPromise: Boolean(connectPromiseRef.current),
      roomState: roomRef.current?.state ?? null
    })

    if (connectPromiseRef.current) {
      console.log("[VOICE-7A] returning existing connect promise")
    ...
    const task = (async () => {
      ...
      try {
        ...
        for (const endpoint of orderedEndpoints) {
          console.log("[VOICE-10] endpoint attempt", {
            url: endpoint.url,
          });
          ...
          try {
            console.log("[VOICE-11] starting ConnectionCheck", {
              url: endpoint.url,
            });
            ...
            console.log("[VOICE-12] ConnectionCheck finished", {
              success: connectionCheck.isSuccess(),
              result: websocketCheck,
            });
        
            if (!connectionCheck.isSuccess()) {
              throw new Error(
                `LiveKit WebSocket недоступен: ${JSON.stringify(
                  websocketCheck,
                )}`,
              );
            }
        
            console.log("[VOICE-13] calling room.connect()");
            ...
            await waitForVoiceMediaConnection(
              room.connect(endpoint.url, credentials.token, {
                ...
                rtcConfig:
                  preferencesRef.current.compatibilityMode
                    ? { iceTransportPolicy: "relay" }
                    : undefined,
              }),
            );
        
            console.log("[VOICE-14] room.connect SUCCESS");
          } catch (cause) {
            console.error("[VOICE-ERROR] endpoint failed", cause);
