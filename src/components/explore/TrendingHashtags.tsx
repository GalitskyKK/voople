...
export function TrendingHashtags() {
  ...
  if (isLoading) {
    return <div className="h-28 animate-pulse rounded-xl bg-[color-mix(in_srgb,var(--foreground)_5%,transparent)]" />;
  }
  ...
  if (data.length === 0) {
    return <p className="text-sm text-[color-mix(in_srgb,var(--foreground)_40%,transparent)]">Пока нет популярных хэштегов</p>;
  }

  return (
    <section className="space-y-3">
      <h2 className="text-sm font-semibold text-[var(--foreground)]">Трендовые хэштеги</h2>
        ...
        {data.map((hashtag) => (
          <li key={hashtag.name}>
            ...
            >
              ...
              <span className="text-xs text-[color-mix(in_srgb,var(--foreground)_45%,transparent)]">{hashtag.postCount}</span>
            </Link>
          </li>
        ))}
      </ul>
