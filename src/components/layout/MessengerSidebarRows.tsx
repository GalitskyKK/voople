    ...
    children: (
      <>
        ...
        <SidebarRowCopy
          ...
          subtitle={
            live
              ? `${live.participantCount} в голосе · ${roomCountLabel(live.roomCount)}`
      ...
      {renderDestination({
        ...
        children: (
          <>
            ...
            {live.roomCount > 1 ? <span className="font-mono text-[11px] leading-none" aria-hidden="true">{live.roomCount}</span> : null}
          </>
        ),
      })}
    </div>
...
function sidebarRowClassName(active: boolean) {
  return cn(
    "voople-messenger-sidebar__row group flex min-h-12 w-full items-center gap-2 rounded-[var(--app-radius-md)] border px-2 py-1.5 text-left transition-colors",
    active
      ? "voople-messenger-sidebar__row--active text-[var(--foreground)]"
      : "border-transparent text-[var(--app-muted)] hover:border-[var(--app-border)] hover:bg-[var(--app-surface-soft)] hover:text-[var(--foreground)]",
  );
}
