...
const SCOPE_OPTIONS: Array<{ value: PrivacyScope; label: string }> = [
  ...
  { value: "contacts_and_groups", label: "Друзья и общие группы" },
  { value: "contacts", label: "Только друзья" },
...
const SCOPE_FIELDS: Array<{ key: keyof Pick<UserPrivacySettingsView, "onlineScope" | "gamingScope" | "musicScope" | "roomsScope" | "inviteScope" | "connectionRequestScope">; label: string }> = [
  ...
  { key: "connectionRequestScope", label: "Кто может отправлять запросы в друзья и новые личные диалоги" },
];

export function UserPrivacySettingsPanel({ load, save, betaOnly = false }: {
