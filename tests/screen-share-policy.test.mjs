...
test("room screen stage owns remaining height and contains remote video", () => {
  ...
  assert.match(stage, /\[&>video\]:h-full/);
  assert.match(stage, /\[&>video\]:w-full/);
  ...
  assert.match(miniStage, /\[&>video\]:h-full/);
  assert.match(miniStage, /\[&>video\]:w-full/);
  assert.match(previewDom, /dataset\.voopleScreenVideo = "true"/);
  assert.match(previewDom, /setProperty\("width", "100%", "important"\)/);
  assert.match(previewDom, /setProperty\("height", "100%", "important"\)/);
